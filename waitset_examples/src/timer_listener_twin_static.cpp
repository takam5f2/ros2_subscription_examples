...
{
// Create a TimerListenerTwinStatic class that subclasses the generic rclcpp::Node base class.
// The main function below will instantiate the class as a ROS node.
class TimerListenerTwinStatic : public rclcpp::Node
{
public:
  WAITSET_EXAMPLES_PUBLIC
  explicit TimerListenerTwinStatic(const rclcpp::NodeOptions & options)
  : Node("timer_listener_twin_static", options)
  {
        ...
        [this]() -> void
    {
      ...
      auto wait_result = static_wait_set_->wait(std::chrono::milliseconds(0));
      ...
      size_t subscriptions_num = static_wait_set_->get_rcl_wait_set().size_of_subscriptions;
    ...
    static_wait_set_ = std::make_shared<rclcpp::StaticWaitSet<2, 0, 0, 0, 0, 0>>(
      ...
      std::array<rclcpp::StaticWaitSet<2, 0, 0, 0, 0, 0>::WaitableEntry, 0>{});
...
private:
  ...
  std::array<rclcpp::Subscription<std_msgs::msg::String>::SharedPtr, 2> subscriptions_array_;

  // StaticWaitSet <Subscription, GuardCondition, Timer, Client, Service, Waitable>
  std::shared_ptr <rclcpp::StaticWaitSet<2, 0, 0, 0, 0, 0>> static_wait_set_;
};

}  // namespace waitset_examples

RCLCPP_COMPONENTS_REGISTER_NODE(waitset_examples::TimerListenerTwinStatic)
